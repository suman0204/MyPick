...
class ShoppingAPIManager {
    ...
    func callRequest(query: String, page: Int, sort: SortType, success: @escaping (Item) -> Void, failure: @escaping () -> Void) {
        ...
        let url = baseURL + encodingQuery + "&display=30" + "&start=\(page)" + "&sort=\(sort)"
